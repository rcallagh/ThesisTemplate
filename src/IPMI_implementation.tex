...
The rest of the paper is organized as follows: \Cref{sec:fibre_growth_algorithm} describes ConFiG, \Cref{sec:experiments_and_results} details some experiments showing the potential of the algorithm and comparing it to a brute-force approach to fibre growth and \Cref{sec:discussion} summarises the contributions and discusses future work.


\begin{algorithm}[h!]
  \begin{algorithmic}
    \Procedure{fibreGrowth}{OD, $\rho$, $d_0$, numNodes}
    \State [startPoints, targets] $\leftarrow$ \Call{getInitialPoints}{OD, $\rho$, $d_0$} \Comment{\Cref{sec:input}}
    ...
    \State\Call{getCandidateNodes}{fibres(i).node(j), DT} \Comment{\Cref{fig:method}a}
    \State \hspace{1em} $\bullet$ candiates $\leftarrow$ DT.nodes sharing edge with fibres(i).node(j)
    \State\Call{getBestStep}{candidates, D, targets}    \Comment{\Cref{fig:method}b}
    ...
    \State\Call{takeStep}{fibres(i), bestStep}       \Comment{\Cref{fig:method}c}
    \State \hspace{1em} $\bullet$ fibres(i).node(j+1) $\leftarrow$ bestStep
    \State\Call{updateTriangulation}{fibres(i), DT, D} \Comment{\Cref{fig:method}d}
    ...
    \EndProcedure
  \end{algorithmic}
  \caption{ConFiG algorithm pseudocode. Takes desired morphological priors (OD, $\rho$ and $d_0$) as well as desired number of nodes in the growth network. From these initial fibre positions, targets and the growth network are generated before the main loop of the algorithm in which each fibre grows one-by-one.}
...
\begin{figure}
	\centering
	\includegraphics[width = \textwidth]{images/newMethod_v1.eps}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{images/metaballs.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{images/substrates_and_signals_otherlabels.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\textwidth]{images/brute_force_vs_algo_2.eps}
