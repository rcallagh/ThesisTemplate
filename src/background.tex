...
\begin{equation}
  ...
  \label{eq:t2star}
\end{equation}
where $\mathrm{T}_2'$ is the relaxation time associated with these external sources. The $\mathrm{T}_2'$ effect can be negated using special MR pulse sequences which will be covered in \Cref{sec:spin_echoes}, however the intrinsic $\mathrm{T}_2$ relaxation cannot be avoided.  
...
\begin{figure}
	...
	\caption{Motion of a spin in the presence of a $B_1$ RF field in the rotating frame. This is identical to the nutation in \Cref{fig:nutation}, however viewing from the rotating frame simplifies the motion. }
	\label{fig:onres}
\end{figure}
...
\begin{figure}
  ...
  \caption{The \acl{FID} described by \Cref{eq:fid}. Here, just the real channel is plotted.}
  \label{fig:fid}
\end{figure}
...
An idealised expression for $\mathbf{\Delta B}(\mathbf{r}(t), t)$ often applied to \ac{MRI} assumes that all of the change in the magnetic field is due to an applied magnetic field gradient, $\mathbf{g}(\mathbf{r}(t), t)$, which only has a significant $\mathbf{\hat{z}}$ component. This means that \Cref{eq:mod_B} can be written as
\begin{align}
...
where $\gamma$ is the gyromagnetic ratio and  $\mathbf{r}(t)$ is the position of the particle. %and $B(\mathbf{r}(t), t)$ is the magnitude of the magnetic field at position $\mathbf{r}(t)$ and time $t$.
...
The first diffusion MR sequence, introduced by Stejskal and Tanner in 1965\cite{Stejskal1965}, is the \ac{PGSE} sequence, shown in \Cref{fig:PGSE_diagram}.
...
The total MR signal comes from an ensemble of spins, each with their own random Brownian motion and thus, from \Cref{eq:delta_phi}, their own phase.
...
In general, we can formulate an equation similar to \Cref{eq:total_signal_sgp} without requiring any assumptions on the gradients as \cite{Price1997,Hall2009}
\begin{equation}
...
Here, the final term describes the evolution of the magnetisation due to a flow, described by a velocity field $\mathbf{v}$. This term is often dropped, assuming no net flow, in the application to \ac{dMRI}. 
...
For instance, the solution to the \ac{BT} equations in isotropic free diffusion can be shown to give the expected Stejskal-Tanner equation, \Cref{eq:stejskal_tanner}.
...
Diffusion simulations attempt to evaluate \Cref{eq:sig_phi_complex} computationally. Simulation approaches broadly fall into two categories: numerical solutions of the \ac{BT} and \ac{MC} simulations of the diffusion dynamics. This section introduces these techniques, highlighting the some of the similarities and differences between them. 
...
In literature, the \ac{BT} equation excluding the relaxation and flow terms is sometimes referred to as the standard \ac{BT} equation and the full \ac{BT} equation in \Cref{eq:bloch_torrey} is referred to as the generalised \ac{BT} equation \cite{Beltrachini2016}. 

Numerical solution approaches combine both the dynamics and the measurement components of the diffusion simulation by solving for the magnetisation in  \Cref{eq:bloch_torrey}.
