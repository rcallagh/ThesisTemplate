...
Not only does this refocus $\mathrm{T}_2^*$ effects as outlined in \Cref{sec:spin_echoes}, but it means that the phase accrued during the second gradient pulse has opposite sign to the phase accrued during the first spin. 
...
The $B_0$ term from \Cref{eq:phi_single_SGP} is the same for both gradient pulses, meaning that during the subtraction in \Cref{eq:delta_phi} it cancels and the only phase shift comes from the diffusive motion.
...
Diffusion simulations attempt to evaluate \Cref{eq:sig_phi_complex} or \Cref{eq:sig_phi_complex} computationally. Simulation approaches broadly fall into two categories: numerical solutions of the \ac{BT} equation and \ac{MC} simulations of the diffusion dynamics. This section introduces these techniques, highlighting the some of the similarities and differences between them. 
